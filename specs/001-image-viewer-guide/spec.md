...
6. **Given** MPR 查看器运行中，**When** 用户在任一视图放置测量工具（长度、角度），**Then** 测量结果显示准确的物理单位（mm、度）
7. **Given** 3D 数据集已加载，**When** 用户调整窗宽窗位，**Then** 调整可以应用到单个视图或所有视图
...
- **FR-024**: 指南 MUST 说明如何在 MPR 视图中集成测量工具（长度、角度、ROI）
- **FR-025**: 指南 MUST 提供完整的 MPR 查看器示例代码，包含所有核心功能
- **FR-026**: 指南 MUST 说明如何处理 MPR 边缘情况（元数据缺失、大数据集性能、越界导航等）
- **FR-027**: 指南 MUST 提供 MPR 性能优化建议（视图更新频率、定位线渲染优化等）

#### 文档质量要求

- **FR-028**: 所有代码示例 MUST 使用中文注释和说明
- **FR-029**: 文档 MUST 使用中文编写，技术术语保留英文并提供中文解释
- **FR-030**: 每个步骤 MUST 清晰明确，避免歧义
- **FR-031**: 文档 MUST 包含目录结构和交叉引用，便于导航
