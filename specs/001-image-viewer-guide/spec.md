...
11. **Given** MPR 查看器已加载多序列 DICOM 数据，**When** 用户打开序列面板，**Then** 显示所有检查和序列的缩略图列表，支持双击切换序列
12. **Given** MPR 查看器正在显示影像，**When** 视口渲染时，**Then** 每个视口的四个角落显示位置信息（切片号、坐标值、窗宽窗位等）
13. **Given** MPR 查看器运行中，**When** 用户拖动测量面板，**Then** 面板跟随鼠标移动，拖到左边缘时自动嵌入到布局中
...
- **FR-032**: 指南 MUST 说明如何实现序列管理功能，包括序列信息提取、缩略图显示、序列切换
- **FR-033**: 指南 MUST 说明如何在视口四角显示信息（切片号、坐标、窗宽窗位等）
- **FR-034**: 指南 MUST 说明如何实现可拖拽和自动嵌入的面板组件

#### 文档质量要求

- **FR-035**: 所有代码示例 MUST 使用中文注释和说明
- **FR-036**: 文档 MUST 使用中文编写，技术术语保留英文并提供中文解释
- **FR-037**: 每个步骤 MUST 清晰明确，避免歧义
- **FR-038**: 文档 MUST 包含目录结构和交叉引用，便于导航
...
- **Series Panel（序列面板）**: 显示所有加载的 DICOM 序列，支持按检查分组和缩略图展示
- **Viewport Overlay（视口叠加层）**: 在视口四角显示位置信息（切片号、坐标、窗宽窗位等）
- **Draggable Panel（可拖拽面板）**: 支持鼠标拖拽和自动边缘吸附的浮动面板组件
