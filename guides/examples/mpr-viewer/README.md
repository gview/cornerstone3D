...
工具栏经过优化，解决了图标和文字相互遮挡的问题：

**优化要点**：
- 使用 `inline-flex` 布局确保图标和文字水平排列
- 添加 `gap: 4px` 在图标和文字之间创建清晰间距
- 增加内边距从 `4px 8px` 到 `4px 10px`
- 图标和文字分离为独立的 `<span>` 元素
...
详细说明请参考：
- 📖 [工具栏优化文档](./TOOLBAR_OPTIMIZATION.md)
...
- 收起时只显示展开按钮，不显示小眼睛图标
- 展开按钮方向根据面板位置自动调整
  - 左侧面板：收起时显示 ▶（向右展开），展开时显示 ◀（向左收缩）
  - 右侧面板：收起时显示 ◀（向左展开），展开时显示 ▶（向右收缩）
...
- 📖 [工具栏优化文档](./TOOLBAR_OPTIMIZATION.md) - 工具栏界面优化说明
